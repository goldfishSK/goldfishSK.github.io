...
jQuery.extend({
    createUploadIframe: function(id, uri) {
        ...
        if (window.ActiveXObject) { if (typeof uri == "boolean") { iframeHtml += ' src="' + "javascript:false" + '"' } else { if (typeof uri == "string") { iframeHtml += ' src="' + uri + '"' } } }
        iframeHtml += " />";
    ...
    createUploadForm: function(id, fileElementId, data, fileElement) {
        ...
        if (data) { for (var i in data) { jQuery('<input type="hidden" name="' + i + '" value="' + data[i] + '" />').appendTo(form) } }
        var oldElement;
    ...
    ajaxFileUpload: function(s) {
        ...
        var uploadCallback = function(isTimeout) {
            ...
            if (xml || isTimeout == "timeout") {
                requestDone = true;
                var status;
                try { status = isTimeout != "timeout" ? "success" : "error"; if (status != "error") { var data = jQuery.uploadHttpData(xml, s.dataType); if (s.success) { s.success(data, status) } if (s.global) { jQuery.event.trigger("ajaxSuccess", [xml, s]) } } else { jQuery.handleError(s, xml, status) } } catch (e) {
                    status = "error";
        ...
        try {
            ...
            if (form.encoding) { jQuery(form).attr("encoding", "multipart/form-data") } else { jQuery(form).attr("enctype", "multipart/form-data") }
            jQuery(form).submit()
        } catch (e) { jQuery.handleError(s, xml, null, e) }
    ...
    handleError: function(s, xml, status, e) { if (s.error) { s.error(xml, status, e) } if (s.global) { jQuery.event.trigger("ajaxError", [xml, s, e]) } }
});
